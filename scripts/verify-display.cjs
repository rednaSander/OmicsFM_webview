...
(async () => {
  ...
  for (const name of fs.readdirSync(root).filter(name=>name.endsWith('.dc.html'))) {
    ...
    assert(runtime.startsWith(fs.readFileSync(path.join(root,'display.js'),'utf8').replace(/\r\n/g,'\n')));
    assert(runtime.includes(fs.readFileSync(path.join(root,'presentation.js'),'utf8').replace(/\r\n/g,'\n')));
